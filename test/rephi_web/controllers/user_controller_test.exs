defmodule RephiWeb.UserControllerTest do
  ...
  setup %{conn: conn} do
    ...
    admin_role = case Authorization.get_role_by_slug("admin") do
      nil -> 
        {:ok, role} = Authorization.create_role(%{
          name: "Administrator",
          slug: "admin",
  ...
  describe "index/2" do
    ...
    test "returns 403 forbidden when authenticated as regular user", %{conn: conn, user_token: user_token} do
      conn = 
        conn
  ...
  describe "show/2" do
    test "shows specific user when authenticated as admin", %{conn: conn, admin_token: admin_token, regular_user: regular_user} do
      conn = 
        conn
        |> put_req_header("authorization", "Bearer #{admin_token}")
    ...
    test "returns 403 forbidden when authenticated as regular user", %{conn: conn, user_token: user_token, admin_user: admin_user} do
      conn = 
        conn
        |> put_req_header("authorization", "Bearer #{user_token}")
  ...
  describe "update/2" do
    test "updates user when authenticated as admin", %{conn: conn, admin_token: admin_token, regular_user: regular_user} do
      updated_email = "updated_email@example.com"
      
      conn = 
        conn
        |> put_req_header("authorization", "Bearer #{admin_token}")
        |> put(~p"/api/users/#{regular_user.id}", %{email: updated_email, password: "newpassword123"})
    ...
    test "returns validation errors with invalid data", %{conn: conn, admin_token: admin_token, regular_user: regular_user} do
      conn = 
        conn
        |> put_req_header("authorization", "Bearer #{admin_token}")
    ...
    test "returns 403 forbidden when authenticated as regular user", %{conn: conn, user_token: user_token, regular_user: regular_user} do
      conn = 
        conn
        |> put_req_header("authorization", "Bearer #{user_token}")
  ...
  describe "delete/2" do
    ...
    test "returns 401 unauthorized when not authenticated", %{conn: conn, regular_user: regular_user} do
