defmodule Mix.Tasks.Rephi.Gen.Frontend do
  ...
  defp update_phoenix_router(app_name) do
    ...
    insert_index = Enum.find_index(lines, fn line ->
      String.contains?(line, "forward \"/example\", RephiWeb.Plugs.FrontendAppPlug, app: \"example\"")
    end)
    
    if insert_index do
      # Insert the new route after the example route
      new_route = "    forward \"/#{app_name}\", RephiWeb.Plugs.FrontendAppPlug, app: \"#{app_name}\""
      ...
      Mix.shell().info("    forward \"/#{app_name}\", RephiWeb.Plugs.FrontendAppPlug, app: \"#{app_name}\"")
    end
  end
    ...
    if String.contains?(content, "example: {") do
      ...
      updated_content = 
        if Regex.match?(~r/example:\s*{[^}]+}\s*}/ms, content) do
          ...
          Mix.shell().warn("⚠️  Could not automatically update env.ts. Please add manually after the example config:")
      ...
      Mix.shell().warn("⚠️  Could not find APPS object in env.ts. Please add the app configuration manually.")
    end
  end
  ...
  defp update_root_package_json(app_name) do
    ...
    scripts_index = Enum.find_index(lines, fn line ->
  ...
  defp get_next_port do
    ...
    ports = Regex.scan(~r/port:\s*(\d+)/, content)
